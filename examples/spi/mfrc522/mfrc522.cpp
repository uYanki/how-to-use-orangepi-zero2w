...
#define MFRC522_RST_PIN 27  // low = power down
#define MFRC522_CS_PIN  16

using namespace std;

#define MFRC522_SPI_CH 1
#define MFRC522_SPI_CS 1

#ifdef MFRC522_CS_PIN
#define MFRC522_Select()   digitalWrite(MFRC522_CS_PIN, LOW)
#define MFRC522_Unselect() digitalWrite(MFRC522_CS_PIN, HIGH)
#else
#define MFRC522_Select()
#define MFRC522_Unselect()
#endif

uint8_t mfrc522_spi_xfer(uint8_t addr)
{
    MFRC522_Select();
    wiringPiSPIDataRW(MFRC522_SPI_CH, &addr, 1);
    MFRC522_Unselect();
    return addr;
}

void mfrc522_spi_xfer_n(char* data, uint8_t size)
{
    MFRC522_Select();
    wiringPiSPIDataRW(MFRC522_SPI_CH, (uint8_t*)data, size);
    MFRC522_Unselect();
}
...
MFRC522::MFRC522()
{
    ...
    pullUpDnControl(MFRC522_RST_PIN, PUD_UP);
    digitalWrite(MFRC522_RST_PIN, HIGH);
#endif
#ifdef MFRC522_CS_PIN
    pinMode(MFRC522_CS_PIN, OUTPUT);
    pullUpDnControl(MFRC522_CS_PIN, PUD_UP);
...
{
    for (
        byte index = 0; index < count; index++)
    {
...
)
{
    ...
    // Reading `count` bytes from register
    byte address = 0x80 | (reg & 0x7E);  // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
    byte index   = 0;                    // Index in values array.
    count--;                             // One read is performed outside of the loop
    mfrc522_spi_xfer(address);
    while (index < count)
    {
        if (index == 0 && rxAlign)
        {  // Only update bit positions rxAlign..7 in values[0]
            ...
            byte mask = 0;
            for (
                byte i = rxAlign; i <= 7; i++)
            {
                mask |= (1 << i);
            }
            // Read value and tell that we want to read the same address again.
            byte value = mfrc522_spi_xfer(address);
            // Apply mask to both current value of values[0] and the new data in value.
            values[0]  = (values[index] & ~mask) | (value & mask);
        }
        else
        {  // Normal case
            values[index] = mfrc522_spi_xfer(address);
        }
        index++;
    }
    values[index] = mfrc522_spi_xfer(0);  // Read the final byte. Send 0 to stop reading.
}
    ...
    while (!uidComplete)
    {
        ...
        while (!selectDone)
        {
            ...
            {  // All UID bits in this Cascade Level are known. This is a SELECT.
                // (F("SELECT: currentLevelKnownBits=")); ln(currentLevelKnownBits, DEC);
            ...
            {  // This is an ANTICOLLISION.
                // (F("ANTICOLLISION: currentLevelKnownBits=")); ln(currentLevelKnownBits, DEC);
 ...
 *
 */
const string MFRC522::GetStatusCodeName(
 ...
 *
 */
const string MFRC522::PICC_GetTypeName(
