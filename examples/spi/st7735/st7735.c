...
{
    digitalWrite(ST7735_RES_Pin, LOW);
    delayMicroseconds(5);
    digitalWrite(ST7735_RES_Pin, HIGH);
}

static void ST7735_WriteCommand(uint8_t cmd)
{
    digitalWrite(ST7735_DC_Pin, LOW);
    wiringPiSPIDataRW(ST7735_SPI_CHANNEL, &cmd, 1);
}

static void ST7735_WriteData(uint8_t* buff, size_t buff_size)
{
    digitalWrite(ST7735_DC_Pin, HIGH);
    wiringPiSPIDataRW(ST7735_SPI_CHANNEL, buff, buff_size);
}
...
{
    wiringPiSetup();
    wiringPiSPISetupMode(ST7735_SPI_CHANNEL, ST7735_SPI_PORT, ST7735_SPI_SPEED, 0);

    pinMode(ST7735_CS_Pin, OUTPUT);
    digitalWrite(ST7735_CS_Pin, HIGH);
    pinMode(ST7735_DC_Pin, OUTPUT);
    digitalWrite(ST7735_DC_Pin, HIGH);
    pinMode(ST7735_RES_Pin, OUTPUT);
    digitalWrite(ST7735_RES_Pin, HIGH);
...
void ST7735_FillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    for (y = h; y > 0; y--)
    {
        for (x = w; x > 0; x--)
        {
            data[0] = color >> 8;
            data[1] = color & 0xFF;
            wiringPiSPIDataRW(ST7735_SPI_CHANNEL, data, sizeof(data));
        }
    }
...
void ST7735_FillRectangleFast(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    uint8_t* line_cpy = malloc(w * sizeof(pixel));
    for (x = 0; x < w; ++x)
    {
    ...
    for (y = h; y > 0; y--)
    {
        memcpy(line_cpy, line, w * sizeof(pixel));
        wiringPiSPIDataRW(ST7735_SPI_CHANNEL, line_cpy, w * sizeof(pixel));
    }
...
void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t* data)
{
    ...
    // 分块发送
    size_t   total_size   = sizeof(uint16_t) * w * h;
    uint8_t  whole_blocks = total_size / DATA_BLOCK_SIZE;
    uint16_t broken_bytes = total_size - whole_blocks * DATA_BLOCK_SIZE;
    ...
    {
        ST7735_WriteData((uint8_t*)data, DATA_BLOCK_SIZE);
        data += DATA_BLOCK_SIZE;
    }

    ST7735_WriteData((uint8_t*)data, broken_bytes);
