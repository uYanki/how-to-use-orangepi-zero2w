...
// Declare
static void portPwmWrite(struct wiringPiNodeStruct* node, int pin, int value);
static void portOnOffWrite(struct wiringPiNodeStruct* node, int pin, int value);
static int  portOffRead(struct wiringPiNodeStruct* node, int pin);
static int  portOnRead(struct wiringPiNodeStruct* node, int pin);
int         baseReg(int pin);
 ...
 */
int pca9685Setup(const char* device, const int pinBase, const int i2cAddress, float freq)
{
    // 使用 Node 挂载虚拟引脚, 以扩展 pwmWrite 等函数
    ...
    // Check i2c address
    int fd = wiringPiI2CSetupInterface(device, i2cAddress);
    if (fd < 0)
    {
        return fd;
    }
    ...
    node->fd           = fd;
    node->pwmWrite     = portPwmWrite;
    node->digitalWrite = portOnOffWrite;
    node->digitalRead  = portOffRead;
    node->analogRead   = portOnRead;
