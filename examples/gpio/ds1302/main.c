// gcc -o app *.c -lwiringPi -lpthread -lwiringPiDev
...
#include <wiringPi.h>
#include <ds1302.h>  // wiringPi 自带库

#define PIN_CLK   13
#define PIN_DAT   15
#define PIN_RST   16  // cs
...
int main(int argc, char* argv[])
{
    ...
    // 若出现 Pin mode failed!, 可尝试直接配置 GPIO 的模式
    ...
    for (i = 0;; ++i)
    {
        ...
        printf("\r");  // 光标回退到行首, 若新输出的字符数量需 >= 已输出的字符数量，则会覆盖先前的输出

        printf("%5d:", i);
        printf("  %04d-%02d-%02d", year, month, date);
        printf("  %02d:%02d:%02d", hour, minute, second);
        printf("  %s", weekdays[weekday - 1]);
